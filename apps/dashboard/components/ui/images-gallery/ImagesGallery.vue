...
const activeImageIndex = ref<number | null>(1),
	activeImage = computed<string>(() => {
		if (
			...
			props.imgUrls !== null &&
			props.imgUrls[activeImageIndex.value]
		)
			return props.imgUrls[activeImageIndex.value];
...
function handlePrevImage() {
	console.log(activeImageIndex.value);
	if (props.imgUrls.length < 2 || activeImageIndex.value === null) return;
	activeImageIndex.value =
		(activeImageIndex.value - 1 + props.imgUrls.length) %
		...
		<div
			...
			<div class="flex items-center justify-center w-full h-full">
				<img
					...
					alt="image"
					loading="lazy"
					class="object-cover h-10 cursor-pointer"
