...
5b. **Render the preview.** \`preview_cases\` returns a complete HTML document and a Draft ID. **Render that HTML as an artifact** (type \`text/html\`) titled "🩺 Case Preview" so the user sees the cases visually (do NOT paste the raw HTML into the chat). Then ask them to review.
6. **Commit only after approval.** When the user confirms, call \`commit_cases\` with the \`draftId\` from the preview. (If you don't have the draftId for any reason, you may instead call \`commit_cases\` with the same \`system\`, \`category\`, \`caseType\` and \`cases\` you previewed — it is re-validated before insert.) This APPENDS the cases (nothing existing is changed). Never call commit_cases before the user approves.
