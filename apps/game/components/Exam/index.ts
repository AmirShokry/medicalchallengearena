...
// import { useGameStore } from "@client/stores";
...
export default function getGameData() {
	...
	const isCorrect = computed(
		...
		correctChoiceIdx = computed(() =>
			cases[current.caseIdx]?.questions[
				current.questionIdx
	...
	const userStatus = computed(() => {
			...
			const lastRecord = user.records.data?.at(
				user.records.data.length - 1
			);
	...
	function getRecordData({
		...
		return {
			...
			questionId:
				cases[current.caseIdx]?.questions[current.questionIdx]?.id!,
	...
	function getCaseIdxAndQuestionIdxFromLastReachedQuestionNumber() {
		...
		for (let i = 0; i < cases.length; i++) {
			if (
				...
				caseIdx = i;
				questionIdx =
					lastReachedQuestionNumber.value - questionNumber - 1;
				break;
			}
			questionNumber += cases?.at(i)?.questions.length!;
		}
