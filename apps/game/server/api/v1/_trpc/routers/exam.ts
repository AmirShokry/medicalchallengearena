...
export const exam = createTRPCRouter({
  ...
  cases: {
    self: authProcedure.query(
      async () => await db.select().from(db.table.cases)
    ),
    studyMore: authProcedure
      .input(z.object({ caseId: z.number().int().min(1) }))
      .query(async ({ input }) => {
        return await db
          .with(CaseQuestionChoicesStudyModeOnlyCTE)
          .select()
          .from(CaseQuestionChoicesStudyModeOnlyCTE)
          .where(eq(CaseQuestionChoicesStudyModeOnlyCTE.id, input.caseId));
      }),
  },
});
